...
**Brand assets — `public/brand/` ships eight files:** `logo-disc.png`, `showbill-lettering.png`, `wordmark-black.png`, `wordmark-white.png`, the three ambient pieces `ambient-emb-1.png` / `ambient-emb-2.png` (corner embroidery) and `ambient-showbill.png` (centered showbill watermark, ≥ 900px, opacity 0.04 — the quietest layer) — owner-sanctioned (2026-07-20), Search-screen desktop ambience only, `#2E1351` tint baked into the PNGs (CSS carries only opacity + placement, static, ≥ 768px) — plus `login-stage-a.jpg`: the Login stage photograph (a MAFIA Cadenza night at the quad, owner-supplied, photographer-credited to Debrato Ghosh, watermark cropped with permission; graded dark with a `#0A0612` violet cast, rendered at 0.5 opacity under black top/bottom gradients + radial vignette on the Login screen only, both viewports, static).
