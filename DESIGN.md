...
**Track 2 — Payment.** Portal-scoped since 2026-07-20 (owner). The pill is orthogonal to journey, but its *arity* depends on who is looking. The ADMIN keeps the full three-state truth (from `paid` / `manuallyVerified`); the INTERVIEWER + desk pill is a TWO-STATE view — Paid or Unpaid — from `paid` ALONE (`deriveInterviewerPayment`, `track="paymentIv"`), because interviewers never need the board's verification state (their job is only to collect the fee; corrections are admin-only).

*Admin — three-state (`track="payment"`; table, drawer, filter chips):*
...
*Interviewer + desk — two-state (`track="paymentIv"`):*

| State | Label | Treatment |
|---|---|---|
| Unpaid | "Unpaid" | **claim amber** ink on claim tint — money outstanding, collect it |
| Paid | "Paid" | good ink on good tint — received (the board verifies separately, admin-only) |

**One color, a portal-scoped meaning.** The SAME claim-amber reads as *"an unverified claim"* on the admin three-state pill and as *"money still outstanding — collect it"* on the interviewer two-state pill. Amber never means "paid" on either. (Interviewer "unpaid" is amber via a track-scoped face in `Pill.css`; the admin "unpaid" stays dormant-neutral — one shared component, two faces.)

**Hard rule:** no UI may ever block, disable, or dim a candidate based on journey state pre-Phase-7. A missing field always renders the permissive state. Amber additionally marks walk-in tickets, the pending-payment chip, the interviewer "unpaid" pill, and the payment-method tag in admin. Green = terminal-good in both tracks. The board's job reads as "drain the amber."

### Payment rails and reconciliation

**Rails (owner truth #4, 2026-07-20).** The Payment ACCOUNT zone offers three rails: the two UPI accounts (each with a QR) and **Cash** — a first-class rail at events, rendered as a third card below the UPI cards. Selecting Cash **hides the QR zone entirely** (cash has no QR) and the hold-to-confirm relabels to "Hold to confirm — ₹300 cash received". Every rail is human-confirmed by the interviewer (owner truth #2 — there is no gateway, no auto-confirm), so the mark-paid claim records **which rail** collected the fee instead of the retired "manual" noise word (`src/paymentClaim.js`, the pure `buildPaymentClaim` — the write-shape guard is `paymentClaim.test.js`):

- **Cash** → `paymentDetails { method: "Cash", timestamp }` — no bank identity.
- **a UPI account** → `paymentDetails { method: <account label>, upiId, upiName, upiType, timestamp }` — the account identity rides along (mirroring the QR-confirm shape).

This is **additive** to the pre-cash write (a bare `{ method, timestamp }`); no field is removed, and `amount` stays absent exactly as before (the drawer defaults it, the export leaves it blank). The result is always the amber "paid · unverified" claim on the **admin** three-state pill; the **interviewer** sees only "Paid" (§5 Track 2) — the board still verifies EVERY claim, cash included. The admin drawer names the rail plainly in the Method row — **"Cash"** or **"UPI · {account}"**, never "manual", never the internal "QR" string (`deriveMethodLabel`); the amber rail tag beside the pill surfaces for every human-confirmed rail (`showsRailTag`) and stays off for the vestigial QR auto-confirm path.

**Phase-8 reconciliation breadcrumb (no code yet — the rule Phase 8 must honor).** A cash claim never lands on a bank statement, so when bank-statement reconciliation ships it MUST **exclude** cash-method claims from the "no bank credit found" fraud list — their absence from the statement is expected, not suspicious — and verify them by **physical cash-count** instead. The tell is `method === "Cash"` **and** the absence of a `upiId` (the UPI branch is the only one that stamps `upiId`); the breadcrumb is pinned in `buildPaymentClaim` and in `paymentClaim.test.js` ("cash claim carries NO upiId — the Phase-8 reconciliation tell").
...
`screen ∈ {login, search, candidate, payment, done}` held in App.js state; Firebase handlers stay in App.js; each screen is a presentational component in `src/screens/interviewer/`. Designed at 375 px, enhanced upward (desktop: 560 px centered column). Global chrome: 48 px TopBar (text wordmark / condensed ticket, sync dot, amber pending-₹ chip, avatar sheet), sticky bottom ActionBar with the screen's ONE primary action, no bottom nav, no counters of any kind on any interviewer surface. Login is the one dark-staged screen (pure `#000000`, ≤ 8 % `--glow-deep` radial, showbill lettering under `mix-blend-mode: screen`); Search/Candidate/Payment/Done are light. Payment confirmation is the green room: a full-viewport takeover legible from two meters — since the celebration pass (owner 2026-07-20) a choreographed one-shot sequence inside the untouched 350 ms crossfade + 2.5 s auto-advance: the drawn tick, the pop with its single ring pulse, the spectrum confetti burst, the ₹300 settling with weight, name/txn rising staggered (the §6 timeline; the celebration frames the facts, never obscures them). Done is composed as a printed receipt on the white paper the owner keeps: mark + name header, hairline rule, Stamp + payment pill body, dashed perforation, actions, wordmark footer — intensity outcome-mapped per §6 (selected: the mild echo with the stamp slam and five drifting flecks; not selected: zero festivity, neutral ink mark, quiet stamp). **The payment flow correction (owner decision 2026-07-20): manual confirm-by-interviewer is the primary payment action; the auto-confirm assumption is retired.** There is no gateway and no bank sync — nothing external ever writes to the 30 s `paymentSessions` poll — so the interviewer, who watches the payment land (club UPI app / candidate's success screen), confirms it: the 600 ms HoldButton ("Hold to confirm — ₹300 received") is the Payment screen's sticky-ActionBar primary, with the {name} + ₹300 restatement visible at the moment of holding; it drives the same mark-paid write the old buried "Other options" sheet did; the interviewer's result pill then reads simply **Paid** (§5 Track 2 is two-state on interviewer surfaces — the board's three-state verification is admin-only), though the confirm evidence still notes the board verifies later. The status line reads the poll honestly ("QR shown — confirm below once you see the payment land · last check m:ss") and never implies automatic confirmation; Cancel payment lives in the bar's ⋯ overflow; Back is the TopBar chevron. The ACCOUNT zone's third rail is **Cash** (owner truth #4): selecting it hides the QR zone (cash has no QR) and relabels the hold to "… ₹300 cash received"; the mark-paid claim records the rail (`method: "Cash"`, or the UPI account label + identity) per §4 "Payment rails and reconciliation".

**The waiting room + recents sidebar (owner feature 2026-07-20):** with the query empty, Search shows the WAITING ROOM — the desk's live queue: candidates with an explicit check-in and no verdict yet (§5 `isWaiting`, an additive VIEW over the same app-level candidates listener the desk writes into; landmine #11 intact — docs without `activated` simply aren't "waiting", nothing gates on it), sorted `activatedAt` ascending so the longest wait is called first (Timestamp/ISO/missing all tolerated; unreadable stamps sort last and settle into place when the serverTimestamp resolves). Standard ResultRow paper cards with the journey pill riding the pill slot beside payment, first 8 + "Show all {N}", section hidden entirely at 0; the header reuses the results-head language and its count is a list-size affordance (the same number as the sanctioned "Show all {N}") — ban #6 stays about progress/throughput counters, which this is not. A desk check-in appears on every interviewer's home in real time. "My recent" keeps the inline section on phones and 768–900px; at ≥ 900px it moves to a collapsible right sidebar on the stage canvas — a fixed 300px translucent night panel under the TopBar whose rows stay the same paper cards, clearing the centered column at ≥ 1280px (below that it may overlay from the right edge, sanctioned). Collapse persists per device under `mafia.recentsSidebar`; collapsed, a slim vertical reopen tab holds the right edge; the swap is the §9 sheet-motion pair (spring entrance, `--ease` exit, transform-only, a 0s-delayed `visibility` flip keeping the off-stage half out of the tab order without unmounting). BOTH placements carry the owner-requested quiet "Clear" (44px target, no confirm — recents are a device-local convenience cache; App.js wipes `mafia.recentCandidates` + state, candidate docs untouched).
...
TWO ways in (name+code added 2026-07-20): (a) a Google account whose verified email is on the `config/checkinDesk { emails: [...] }` allowlist — App.js resolves the role LIVE from a single-doc onSnapshot; signed-in sessions hold the boot splash until the first role snapshot so neither portal ever flashes for the wrong role; or (b) the low-friction **name + code** path off the Login screen ("I'm at the check-in desk" → a name field + the shared `deskCode` → Enter): App.js opens a silent Firebase **anonymous** session (`signInAnonymously`), reads `config/checkinDesk` to validate the code, and on a match stashes the typed name (per-tab `sessionStorage` "mafia.deskName") as the session's whole identity — a wrong code signs the throwaway session straight back out to an inline Banner. An anonymous session with a validated name IS the desk user (it drives attribution + the role); without one it never reaches any portal. Google interviewers are untouched — Google stays the primary CTA. A desk account renders `src/screens/desk/Desk.jsx` instead of the interviewer state machine — Candidate/Payment/verdict surfaces are simply never routed for them; the only reachable writes are the additive check-in fields (`activated`, `activatedAt`, and the additive `activatedBy` attribution + updated-meta — the anonymous session having no email, `activatedBy`/`lastUpdatedBy` carry the typed desk name so the admin activity feed shows who worked the door) through the same merge-update path the admin check-in uses, a scope now **rule-enforced** for anonymous sessions (see below). The screen is the front desk's calling sheet: full live candidate list (search narrows, first 30 + "Show more", slotOrder-then-name ordering), paper card rows (name · mono regNo · year · journey pill · tap-to-call `tel:` WhatsApp link · slot chip when the time-slot layer has data) with ONE action, "Check in"; checked-in rows show the quiet mono "✓ Checked in {time}" whose tap asks "Mark as not arrived?" (ConfirmPopover → explicit `activated:false`, the honest Registered state). Row tap opens the FULL read-only record in the right Drawer (admin record-body patterns: identity dl, rank-chip preferences, comments — no verdict controls, no payment block). Sticky under the TopBar: live "Checked in X / Y" + amber "Waiting {N}" (`isWaiting` — explicitly checked in, no verdict; an additive §5 VIEW, never a gate). **The desk counts are sanctioned:** §9 ban #6 is scoped to interviewer surfaces, and the count IS the desk's job. Aesthetic: the stage canvas language — dark canvas ≥ 900px with stage inks on canvas-level text (the amber Waiting count rides its own claim-tint chip so ink never sits on the canvas), paper below; rows stay paper cards on both. Admin manages both entry paths from the ⋯ overflow ("Check-in desk…" Dialog): the live add/remove email allowlist, and a **Rotate code** action generating a fresh unambiguous 6-char `deskCode` (`src/deskCode.js` — pure, no ambiguous `0/O/1/I/L`, `codesMatch` trims + case-folds and never lets a blank pass). Both writes MERGE `config/checkinDesk`, so the code and the allowlist never clobber each other; rotating retires the previous code (already-signed-in desk sessions stay — the code gates sign-IN, not the live session).

**Role-gating honesty (recorded, not negotiable):** the *interviewer/admin* split remains UI-level separation on the existing trust model — any signed-in verified college account can technically write to Firestore surfaces beyond its portal; `config/checkinDesk`'s email allowlist ROUTES those roles, it does not enforce them. What IS now rule-enforced (2026-07-20): the **anonymous desk** write-scope. `firestore.rules` `isDesk()` (a signed-in anonymous session — `sign_in_provider == 'anonymous'`) may READ candidates + `config`, and UPDATE a candidate ONLY when `request.resource.data.diff(resource.data).affectedKeys().hasOnly(['activated','activatedAt','activatedBy','lastUpdatedBy','lastUpdatedAt'])` — verdict, payment, identity, create and delete are unreachable for it at the rules layer, not merely in the UI. The trade the owner accepted: the `deskCode` doc AND the candidate roster are readable by any anonymous session, so the code gates the check-in *UI* while the rule gates what a desk session may *write*. Enabling Anonymous auth in the Firebase console + deploying `firestore.rules` are owner console actions; until both land, the name+code path fails gracefully to an inline Banner.

### Admin (`/admin`) — laptop-first, one screen + drawer

Entire tree (login included) wrapped in `data-theme="dark"`. One honest screen, no sidebar: topbar (white lockup, 2 px six-segment spectrum hairline, ticking "Synced Ns ago" pill, Export with scope popover, overflow menu) → 4-tile stat strip (auto-fit `minmax(180px,1fr)`) → candidates table card (40 px SearchField, filter chips with live counts, 14 px text / 44 px rows, sticky header, hand-rolled IntersectionObserver windowing at 100-row pages) beside the interviewers + activity cards at ≥ 1280. Row click opens the 420 px right Drawer headed by the compact Ticket — verify/reverse actions sit next to the evidence, guarded by ConfirmPopover / Dialog. At 768–1024: single-column stack, table scrolls horizontally inside its card, drawer becomes a full-screen overlay. Danger zone: amber `RESET` dialog vs red `DELETE` dialog, both `busy`-held open. The ⋯ overflow's two operational dialogs: **"Check-in desk…"** (the desk allowlist) and **"Import time slots…"** (stage B, 2026-07-20) — a fully client-side parse of the front desk's slot sheet (.xlsx via the already-shipped ExcelJS READ path, .csv via the pure `src/slotImport.js`; tolerant header autodetect ~ reg/registration/id + slot/time, normalized-regNo matching with a unique-name fallback), then a preview that states exactly what Apply will write ("N matched · M unmatched", duplicates last-wins, unmatched rows listed in a mono well) before batched ADDITIVE writes of `slot` + `slotOrder` (≤ 400/batch; deliberately no meta stamping so a 600-row import can't flood the activity feed or trip undo stale-checks). The export mapping (`buildExportRows`) is untouched — landmines #9/#10 hold.
